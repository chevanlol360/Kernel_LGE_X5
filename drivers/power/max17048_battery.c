...
#if defined(CONFIG_MACH_MSM8974_G2_KR) || defined(CONFIG_MACH_MSM8974_VU3_KR) || defined(CONFIG_MACH_MSM8974_B1_KR) || defined (CONFIG_MACH_MSM8926_B2L_ATT)
/* using to cal rcomp */
int cell_info = 0;
#endif
...
#elif defined(CONFIG_MACH_MSM8926_B2L_ATT)
	batt_soc = (batt_soc-((ref->model_data->empty)*100000))
						/(10000-(ref->model_data->empty))*10000;
#else
		...
		struct max17048_battery_model *mdata)
{
	...
	if(cell_info == LGC_LLL) {
		...
		rc = of_property_read_u32(dev_node, "max17048,temp_co_hot_l",
				&mdata->temp_co_hot);
		rc = of_property_read_u32(dev_node, "max17048,temp_co_cold_l",
				&mdata->temp_co_cold);
		...
		rc = of_property_read_u32(dev_node, "max17048,temp_co_hot_d",
				&mdata->temp_co_hot);
		rc = of_property_read_u32(dev_node, "max17048,temp_co_cold_d",
				&mdata->temp_co_cold);
		rc = of_property_read_u32(dev_node, "max17048,empty_d",
			&mdata->empty);
	}
#else
...
/* Parsing Device Tress or Using Device Platform data */
	...
	if (chip->model_data->alert_gpio) {
		...
		gpio_tlmm_config(GPIO_CFG(chip->model_data->alert_gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_4MA), GPIO_CFG_ENABLE);
...
#ifdef CONFIG_LGE_PM
#if 0
	chip->maxim17048.name		= "max17048";
	chip->maxim17048.type		= POWER_SUPPLY_TYPE_FUELGAUGE;
	chip->maxim17048.get_property	= max17048_get_property;
	chip->maxim17048.properties	= max17048_battery_props;
	chip->maxim17048.num_properties	= ARRAY_SIZE(max17048_battery_props);
#endif
...
#ifdef CONFIG_LGE_PM
int32_t max17048_is_ready(void)
{
	struct max17048_chip *maxim17048_data = ref;

	if (!maxim17048_data)
		return -EPROBE_DEFER;
	return 0;
}
EXPORT_SYMBOL(max17048_is_ready);
#endif

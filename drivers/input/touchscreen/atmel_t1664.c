...
{
#if 0 // Cannot update Touch, please check status value.
	...
	if (status & MXT_T100_SUPPRESSION) {
		if (touched_finger_count) {
			TOUCH_INFO_MSG(" MXT_T100_SUPPRESSION\n");
			mxt_reset_slots(data);
		}
		if (data->palm) {
			cancel_delayed_work_sync(&data->work_palm_unlock);
...
//	struct device *dev = &data->client->dev;
	...
	data->update_input = true;
#endif // Cannot update Touch, please check status value.
}
...
//soyun	set_bit(EV_ABS, input_dev->evbit);
//soyun	input_set_capability(input_dev, EV_KEY, BTN_TOUCH);
...
#if 0 // Do not upgrade on Rev A SMT
...
#endif // Do not upgrade on Rev A SMT

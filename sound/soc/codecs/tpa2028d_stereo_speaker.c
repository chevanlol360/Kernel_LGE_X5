...
inline void set_amp_gain(int amp_no, int amp_state)
{
	...
	case SPK_ON:
		...
		if (amp_data[amp_no]->pdata->enable)
			...
			fail = amp_data[amp_no]->pdata->enable(1, amp_data[amp_no]->pdata->enable_gpio);
#else
	...
	case SPK_OFF:
		if (amp_data[amp_no]->pdata->enable)
#ifdef CONFIG_SND_SOC_TPA2028D_E10WIFI
			fail = amp_data[amp_no]->pdata->enable(2, amp_data[amp_no]->pdata->enable_gpio);
#else
		...
		if (amp_data[amp_no]->pdata->enable)
#ifdef CONFIG_SND_SOC_TPA2028D_E10WIFI
			fail = amp_data[amp_no]->pdata->enable(0, amp_data[amp_no]->pdata->enable_gpio);
#else
...
#ifdef CONFIG_SND_SOC_TPA2028D_E10WIFI
int amp_enable(int on_state, int enable_gpio)
{
	int err = 0;
